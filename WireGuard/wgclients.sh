...
# wget -qO- https://git.io/pxuwgc | bash
# 使用url调用脚本, 或者下载脚本使用, 可以指定端口数
# bash <(curl -L -s https://git.io/pxuwgc) 32001
#############################################################

let port=$RANDOM/2+32001
mtu=1420
...
wget -qO- git.io/pxydwg | bash

# 一键安装wireguard 脚本 Ubuntu   (源:逗比网安装笔记)
wget -qO- git.io/pxyuwg | bash

# CentOS 7 一键脚本安装WireGuard  (官方脚本自动升级内核)
wget -qO- git.io/pxycwg | bash
...
echo -e "# ${Info} 自定端口${RedBG} bash <(curl -L -s https://git.io/pxywgc) ${GreenBG} 32001 ${Font}"
